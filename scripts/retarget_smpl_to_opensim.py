...
def write_process_log(log_path: Path | None, output: str) -> None:
    if log_path is None:
        return
...
def run_opensim(opensim_cmd: str, setup_xml: Path, log_path: Path | None = None) -> None:
    ...
    result = subprocess.run(cmd, cwd=str(setup_xml.parent), capture_output=True, text=True)
    sys.stdout.write(result.stdout)
    sys.stderr.write(result.stderr)
    write_process_log(log_path, result.stdout + result.stderr)
