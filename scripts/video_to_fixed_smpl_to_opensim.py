...
def build_opensim_feedback_cmd(
...
def main() -> int:
    ...
    if args.opensim_feedback_loop and (
        not args.world_grounded or not args.optimize_lower_body
    ):
        parser.error(
            "--opensim-feedback-loop requires --world-grounded and --optimize-lower-body."
        )
    ...
    lower_body_corrected_smpl_pkl = None
    ...
    if args.optimize_lower_body:
        ...
        run(lb_cmd)
        lower_body_corrected_smpl_pkl = lower_body_out / "corrected_smpl.pkl"
        retarget_input_pkl = require_file(
            lower_body_corrected_smpl_pkl, "lower-body corrected SMPL pkl"
        )

    selected_corrected_smpl_pkl = None
    if args.opensim_feedback_loop:
        stage6_input_pkl = world_grounded_out / "optimized_canonical_wham_output.pkl"
        require_file(stage6_input_pkl, "world-grounded optimized pkl")
        run(build_opensim_feedback_cmd(args, stage6_input_pkl, opensim_feedback_out))
        selected_corrected_smpl_pkl = (
            opensim_feedback_out / "selected_corrected_smpl.pkl"
        )
        retarget_input_pkl = require_file(
    ...
    if args.optimize_lower_body:
        ...
        print(f"corrected_smpl_pkl: {lower_body_corrected_smpl_pkl}")
    if args.opensim_feedback_loop:
